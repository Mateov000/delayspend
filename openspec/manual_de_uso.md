...
### 10.3 Resiliencia en iOS / iPhone (WebKit) y Reconexión Inteligente
Los dispositivos iOS (Safari y PWAs agregadas a la pantalla de inicio) presentan una particularidad técnica: cuando el teléfono se bloquea, entra en reposo o pierde cobertura celular momentáneamente, el kernel congela las conexiones TCP existentes dejándolas en un estado "medio abierto" (*half-open*). En navegadores tradicionales sin protección, esto provocaba que una sincronización quedara esperando indefinidamente una respuesta de red que nunca llegaba, mostrando un cartel perpetuo de *"Sincronizando..."*.

DelaySpend resuelve esto mediante una arquitectura multicapa de alta tolerancia a fallos:
1. **Timeouts Estrictos (Fail-Fast):** Todas las consultas de subida y descarga a Supabase están envueltas en un límite de tiempo de 8 segundos (y 6 segundos en operaciones puntuales). Si la red se cuelga o no responde, la petición se cancela limpiamente en lugar de trabar la aplicación.
